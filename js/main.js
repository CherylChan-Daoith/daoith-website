...
function stripKnowledgeBaseDisclaimers(text) {
  ...
  t = t.replace(
    /(?:^|\n)\s*(?:结论\s*[:：]\s*)?知识库(?:尚未收录|未检索到|未命中|无精确匹配|无该条目|中并无|中没有)[^。；\n]*[。；]?\s*/g,
    '\n'
  );
  // Soft leaks: 「知识库中并无逐条清单…」「知识库里没有具体门槛…」
  t = t.replace(
    /[^。；\n]*知识库[^。；\n]*(?:并无|没有|未收录|未命中|未检索|无逐条|找不到|未覆盖|不含|未包含)[^。；\n]*[。；]?\s*/g,
    ''
  );
...
function isKnowledgeMissRefusal(text) {
  const t = String(text || '');
  return /知识库尚未收录|知识库未检索到|知识库未命中|目前的知识库尚未|未在知识库|知识库中并无|知识库中没有|无逐条清单|建议查阅官方税务局|信息还需补充，或者预约专家/.test(
    t
  );
}
