...
function formatDiagSlotsForApi(slots) {
  ...
  if (exportMode === '由平台安排出口') {
    hard += '出口已确定为「由平台安排出口」，禁止写出口未知，方案只围绕平台统一安排出口。\n';
  }
  if (/国内仓/.test(shipping) && !/保税仓/.test(shipping)) {
    hard += `发货必须写「${shipping}」，禁止改成保税仓。\n`;
  }
  if (/保税仓/.test(shipping)) {
    hard += `发货必须写「${shipping}」，禁止改成国内仓。\n`;
  }
  if (revenue && revenue !== '未填写') {
    hard += `年销售额必须写「${revenue}」，禁止改用其它档位。\n`;
  }
  hard += `样本仅供结构参考，禁止照抄示例画像。本单以档案为准：${platform} / ${shipping} / ${exportMode} / ${revenue}。`;
...
function buildDiagnosisPlanApiQuery(userText) {
  ...
  return (
    '【专属合规诊断·生成报告】第1-7步已齐。禁止再提问。\n' +
    `${archive}\n` +
    (reply ? `【用户本轮最后答复】${reply}\n` : '') +
    '出报告前必须先通读《出报告硬约束与路径要点》和《必须知道的知识点》（含问题1–7各选项注意事项，按档案对照），再按需检索命中方案样本。' +
    '报告格式与生成内容要求以《出报告硬约束与路径要点》为准；核心判断采信《必须知道的知识点》。档案字段必须逐字采信。'
  );
}
...
function buildDiagnosisFollowUpQuery(userText, baselineSlots, changes) {
  ...
  return (
    ...
    '- 若用户明确改了业务条件（陈述句）：先写【变化点】（旧→新），再写【影响与注意事项】，然后按新事实重出报告（重出前通读《出报告硬约束与路径要点》《必须知道的知识点》）；新事实覆盖旧档案。\n' +
    '- 若为全新无关问题：按类型B作答，勿套用旧报告。'
  );
}
...
function buildDiagnosisApiQuery(text, uiMode, uiStep, platformLabel, options = {}) {
  ...
  const stepHints = {
    ...
    8: '第1-7步已齐。出报告前必须先通读《出报告硬约束与路径要点》和《必须知道的知识点》（含问题1–7各选项注意事项），再基于【诊断档案】输出报告；不要再提问，不要声称信息缺失，不要检索独立问题X分篇。',
  };
