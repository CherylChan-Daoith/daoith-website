  ...
  window.DAOITH_SERVICES = [
    ...
    },
    {
      ...
      details: [
        ...
        {
          type: 'p',
          text: '面向跨境电商企业的合规代账全托管服务：从业务摸底、存量风险筛查、数据交接，到月度账务与税务申报、季度经营分析及年度健康体检，数字化全流程交付，并可按需绑定出口退税协同管理。',
        },
        {
          type: 'table',
          firstColHeader: true,
          headers: ['服务项目', '合规代账托管', '传统代账', '服务价值点'],
          rows: [
            [
              '业务数据处理方式',
              '全流程数字化处理，系统自动抓取，减少人工干预',
              '手工录入原始单据，人工录入比例高',
              '保证数据及时性、准确性，降低错漏风险',
            ],
            [
              '核算颗粒度',
              '按订单、按项目、按SKU进行核算，深度还原业务',
              '简单流水账核算，仅满足报税需求',
              '支持经营决策，清楚每一笔业务的利润情况',
            ],
            [
            ...
            [
              '沟通协作深度',
              '专属顾问团队，深入了解业务，提供针对性方案',
              '机械收单，缺乏有效沟通',
              '建立互信关系，成为企业不可或缺的外部财务部',
            ],
            [
              '档案数字化管理',
              '电子会计档案，随时随地查阅，永久保存',
        ...
        {
          type: 'p',
          text: '按年销售额每 500 万人民币一档计价，服务费按年收取。不含出口退税按销售额 0.1% 收取（封顶 ¥30,000）；含出口退税按销售额 0.2% 收取（封顶 ¥60,000）。以下为常用档位示例：',
        },
        {
          type: 'table',
          firstColHeader: true,
        ...
        {
          type: 'p',
          text: '报价不含政府规费、税控设备、第三方审计及加急窗口费用；最终以签约方案为准。多主体、多店铺或历史账务重建可按工作量另计。',
        },
        { type: 'h2', text: '服务流程和时效' },
        {
          type: 'timeline',
          steps: [
            { title: '了解业务现状', time: '付款后一周内', desc: '访谈调研，梳理业务与资金流' },
            { title: '财税风险筛查', time: '服务开始前半年', desc: '排查存量风险并形成整改路径' },
            { title: '数据交接及准备', time: '付款后1个月内', desc: '资料交接、账务初始化确认' },
            { title: '账务处理', time: '每月', desc: '数字化记账与凭证归档' },
            { title: '税务申报', time: '每月及每季度', desc: '各税种申报与完税凭证回传' },
            { title: '财税分析', time: '每季度', desc: '经营分析与异常预警解读' },
            { title: '年度健康体检汇报', time: '每年1次', desc: '汇算清缴与合规综合评估' },
            { title: '账务备查文档检查', time: '年度', desc: '全套档案整理与审计配合' },
          ],
        },
        {
          type: 'table',
          firstColHeader: true,
          headers: ['阶段', '服务内容', '服务周期', '交付物'],
          rows: [
            ['1', '业务现状了解', '付款后一周', '《访谈提纲及调研报告》《业务流程梳理图》'],
            ['2', '财税风险筛查', '服务开始前半年', '《存量财税风险评估报告》《风险整改方案》'],
            ['3', '数据交接及准备', '付款后1个月内', '《资料交接清单》《账务初始化数据确认表》'],
            ['4', '合规代账账务处理', '每月', '《记账凭证》《会计账簿》、月度三大表'],
            ['5', '合规代账税务申报', '每月／每季度', '《各税种申报表》《完税证明／缴款凭证》'],
