...
同步把 `data/compliance-must-read-md/` 里的《出报告硬约束与路径要点》《必须知道的知识点》更新到知识库「合规解决方案必读文件」。问题1–7已并入《必须知道的知识点》，上传后请在 Dify 里**停用或删除**旧的「问题X+关键词」分篇，避免抢检索。不更新知识库则出报告会缺约束。
...
- **`合规解决方案必读文件`**（出报告前强制通读：《出报告硬约束与路径要点》《必须知道的知识点》（含问题1–7选项注意事项）；路径 A/B/C 方案样本作参考）
...
### D. 诊断完成后追问（变化识别）
先走完 7 步并出报告（例如上一轮产品为 **0 退税率**）。再发：

`Amazon平台销售商品，注册主体为中国境内公司，发货模式国内直发，货物通过正式报关出口，上游供应商能开具普通发票，产品属于13%退税率产品，年销售5000万`

期望：
- 先列出与上一轮的变化点（至少包含退税率 0% → 13%）
- 分析该变化对退税可行性、架构与税负的影响及注意事项
- 按 **13% 可退税（普货）** 重写方案，禁止仍写「0 退税率」
- 若再问无关问题（如「德国 VAT 怎么注册」），不套用上一份报告，全新作答

官网侧：诊断完成后不要再把新问题包装成「第1-7步已齐，请基于旧档案出报告」。
...
- [ ] 退税率不与增值税混淆  
- [ ] 知识库无关片段不会串台  
- [ ] 诊断完成后追问能识别变化点；相关则更新方案，全新问题则全新作答  
- [ ] 已 **发布**（草稿预览 ≠ 官网 API）
