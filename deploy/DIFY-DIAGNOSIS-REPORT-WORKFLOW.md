...
1. Agent 节点 → **工具** → 添加 **知识库**（Dataset）——可挂 **多个** 库（按你 Dify 里实际名称勾选）
2. **出报告建议勾选（与问诊 Agent 历史一致）**：

| 优先级 | 知识库（名称以工作室为准） | 用途 |
|--------|---------------------------|------|
| **必挂** | `合规解决方案必读文件` | 硬约束、知识点、问题1–7 分篇、路径 A/B/C/D 方案样本；（若实操附件也在此库则一并覆盖） |
| **建议挂** | `跨境电商实操知识`（若独立成库；有的环境已并入必读库） | 主体选择、9610/9810、增值税法影响等附件 |
| **建议挂** | `海关编码和出口退税率` | 产品归类 / 0 退税率 / 特殊商品标识；严守退税率≠增值税 |
| **按需挂** | `出口退税法规和实操` | 退税法规与实操补强 |
