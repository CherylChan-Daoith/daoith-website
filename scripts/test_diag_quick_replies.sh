...
"""
    ...
    + r"""
const cases = [
  { name: "welcome-modeSelect-stale-step3", text: "请在下方选择：生成专属合规方案（需微信登录，按步骤生成合规方案），或 特定问题直接咨询（基于知识库即时解答）。", mode: "diagnosis", step: 3, platform: "亚马逊 Amazon", expect: "modeSelect" },
  { name: "welcome-modeSelect-fresh", text: "请在下方选择：生成专属合规方案（需微信登录，按步骤生成合规方案），或 特定问题直接咨询（基于知识库即时解答）。", mode: "", step: 0, platform: "", expect: "modeSelect" },
